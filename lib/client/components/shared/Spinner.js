...
var Spinner = (function (_React$Component) {
  ...
  _createClass(Spinner, [{
    ...
    value: function render() {
      var quote = this.props.quote || this.props.defaultQuotes[Math.floor(Math.random() * this.props.defaultQuotes.length)];
      var spinnerMainId = this.props.fullScreen ? 'full-screen-spinner' : '';
      var spinnerRectClass = this.props.fullScreen ? 'spinner-rect' : '';

      if (this.props.fullScreen) {
        return _react2['default'].createElement(
          'div',
          { className: 'full-screen-spinner-wrapper' },
    ...
    key: 'defaultProps',
    value: {
      defaultQuotes: ['Money isn\'t important, but having it, that\'s a different question.', 'What do you call it when Batman skips church? Christian Bail.', '3 A.M. Phone Calls: "Are you asleep?", Me: "No I\'m skydiving..."', 'Laughter is the best medicine, but if you\'re laughing for no reason, you probably need medicine.', 'Knowledge is knowing tomatos are a fruit. Wisdom is knowing not to put tomatos in a fruit salad.']
    },
    enumerable: true
  }]);
