...
var NewTimesheetStore = Reflux.createStore({
  ...
  onSetWorkType: function onSetWorkType(workType) {
  ...
  onSetNote: function onSetNote(note) {
    this.state.timesheet.note = note;
    this.trigger(this.state);
  },
  onSetTime: function onSetTime(hours, minutes) {
    var valid = true;
    var validations = [InputValidator.validateStringPresence('Please fill out your work duration', hours, minutes), InputValidator.validateIntegerOnly(null, hours, minutes), InputValidator.validateLength('Please fill out all digits (ie. 00:00)', 2, hours, minutes), CustomValidator.validateCorrectTimeInput(hours, minutes)];
    ...
    if (valid) {
      ...
      this.state.timesheet.timeInSeconds = this._formatHoursToSeconds(hours) + this._formatMinutesToSeconds(minutes);
      this.trigger(this.state);
    }
  },
  onSubmitTimesheet: function onSubmitTimesheet() {
    this.state.creatingTimesheet = true;
    TimesheetActions.addTimesheet(this.state.timesheet);
    this.trigger(this.state);
  },
  onFinishCreatingTimesheet: function onFinishCreatingTimesheet() {
