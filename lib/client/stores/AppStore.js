'use strict';

var Reflux = require('reflux');
var _ = require('lodash');
var AppActions = require('.././actions/AppActions');

var AppStateTemplate = {
  currentUser: null,
  apiToken: null,
  modal: {
    newTimesheet: false,
    newNote: false,
    newGroup: false
  },
  dashboardReady: false,
  flash: {
    message: null,
    error: null
  }
};

var AppStore = Reflux.createStore({
  init: function init() {
    this.state = _.cloneDeep(AppStateTemplate);
    this.listenToMany(AppActions);
  },
  getState: function getState() {
    return this.state;
  },
  onToggleModal: function onToggleModal(modalName) {
    // first sets all modals to false, and then sets true to inputted modal name is there is one
    this.state.modal = _.mapValues(this.state.modal, function (v) {
      return v = false;
    });
    if (modalName) this.state.modal[modalName] = true;
    this.trigger(this.state);
  },
  onLoadInitialDashboardDataCompleted: function onLoadInitialDashboardDataCompleted() {
    this.state.dashboardReady = true;
    this.trigger(this.state);
  },
  onTriggerFlashMessage: function onTriggerFlashMessage(message) {
    this.state.flash = { error: null, message: message };
    this.trigger(this.state);
  },
  onTriggerFlashError: function onTriggerFlashError(error) {
    this.state.flash = { error: error, message: null };
    this.trigger(this.state);
  }
});

module.exports = AppStore;